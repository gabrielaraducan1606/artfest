...
const shouldOpenPolicyGate =
  ...
  params.get("policyGate") === "true";
const queryPolicyScope = params.get("scope") || null;
  ...
  const cachedCounts = useMemo(
    () => readCache(COUNTS_CACHE_KEY, 20_000),
    []
  );
  ...
  const [unreadNotif, setUnreadNotif] = useState(
    cachedCounts?.unreadNotif ?? 0
  );
  const [unreadMsgs, setUnreadMsgs] = useState(
    cachedCounts?.unreadMsgs ?? 0
  );
  const [cartCount, setCartCount] = useState(cachedCounts?.cartCount ?? 0);
  const [wishlistCount, setWishlistCount] = useState(
    cachedCounts?.wishlistCount ?? 0
  );
  const [userUnreadMsgs, setUserUnreadMsgs] = useState(
    cachedCounts?.userUnreadMsgs ?? 0
  );
  const [supportUnread, setSupportUnread] = useState(
    cachedCounts?.supportUnread ?? 0
  );
...
useEffect(() => {
  ...
  }, [cachedMe, location.pathname, location.search]);
  ...
  const [revTab, setRevTab] = useState("sent");
  ...
  async function loadReviews(page = 1, replace = false) {
    try {
      ...
      const [sent, recv] = await Promise.all([
        api(`/api/desktop-reviews/my?page=${page}&limit=${limit}`).catch(
          () => ({ items: [] })
        ),
        isVendor
          ? api(
              `/api/desktop-reviews/received?page=${page}&limit=${limit}`
            ).catch(() => ({ items: [] }))
          : { items: [] },
      ]);
  ...
  async function loadComments(page = 1, replace = false) {
    try {
      ...
      const [sent, recv] = await Promise.all([
        api(`/api/product-comments/my?page=${page}&limit=${limit}`).catch(
          () => ({ items: [] })
        ),
        isVendor
          ? api(
              `/api/product-comments/received?page=${page}&limit=${limit}`
            ).catch(() => ({ items: [] }))
          : { items: [] },
      ]);
      ...
      >
        ...
        <section className={styles.section}>
          ...
          <div className={styles.card}>
            ...
            <RowExternalLink
              href={`${docsBase}/termenii-si-conditiile`}
              label="Termeni și condiții"
              icon={<FileText size={20} />}
            />

            <RowExternalLink
              href={`${docsBase}/confidentialitate`}
              label="Politica de confidențialitate"
              icon={<ShieldHalf size={20} />}
            />

            <RowExternalLink
              href={`${docsBase}/politica-retur`}
