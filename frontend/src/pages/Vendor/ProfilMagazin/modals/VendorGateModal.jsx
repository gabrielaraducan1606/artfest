...
export default function VendorGateModal({
  ...
  const productsAddendumUrl =
    legalHref(
      gateDocs?.products_addendum
        ?.url ||
        "/anexa-produse"
    );
  ...
  const returnsPolicyUrl =
    legalHref(
      gateDocs?.returns_policy_ack
        ?.url ||
        gateDocs?.returns?.url ||
        "/politica-retur"
    );

  const shippingAddendumUrl =
    legalHref(
      gateDocs?.shipping_addendum
        ?.url ||
        "/anexa-expediere"
    );

  const productsAddendumVersion =
    gateDocs?.products_addendum
      ?.version ||
    "";
  ...
  const returnsPolicyVersion =
    gateDocs?.returns_policy_ack
      ?.version ||
    gateDocs?.returns?.version ||
    "";

  const productDeclVersion =
    gateDocs?.product_declaration
      ?.version ||
    "1.0.0";
  ...
  const errors = useMemo(() => {
    ...
    if (
      ...
      list.push(
        "Completează adresa pentru retururi."
      );
    }

    if (!phoneValue) {
      list.push(
        "Completează telefonul pentru retururi."
      );
    }

    if (
      phoneValue &&
      !isPhoneIntl(phoneValue)
    ) {
      list.push(
    ...
    if (!emailValue) {
      list.push(
        "Completează emailul pentru retururi."
      );
    }
    ...
    if (
      ...
      list.push(
        "Emailul pentru retururi nu este valid."
      );
    }
    ...
    if (
      ...
      list.push(
        "Confirmă declarația privind produsele."
      );
    }
  ...
  function updateShipping(
    ...
    /*
     * Ștergem eroarea generală
     * când utilizatorul începe
     * să corecteze formularul.
     */
    if (localErr) {
      setLocalErr(null);
    }
  ...
  async function saveShippingData() {
    if (!serviceId) {
      throw new Error(
        "Lipsește serviceId pentru salvarea datelor de livrare."
      );
    }
  ...
  const handleConfirm =
    async () => {
      if (!serviceId) {
        setLocalErr(
          "Lipsește serviceId. Nu pot salva datele de livrare."
        );
      ...
      if (
        ...
        setLocalErr(
          `Te rugăm să corectezi următoarele:\n• ${errors.join(
            "\n• "
          )}`
        );
      ...
      try {
        ...
        if (
          ...
          setLocalErr(
            "Nu am putut salva datele de livrare. Verifică informațiile introduse și încearcă din nou."
          );
        } else if (
          message ===
          "no_active_policy"
          ...
          setLocalErr(
            "Politica legală necesară nu este disponibilă momentan. Te rugăm să încerci din nou puțin mai târziu."
          );
        } else if (
          message ===
          "product_declaration_failed"
          ...
          setLocalErr(
            "Nu am putut salva declarația privind produsele. Te rugăm să încerci din nou."
          );
        } else if (
          message ===
          "vendor_accept_failed"
          ...
          setLocalErr(
            "Nu am putut salva acceptarea politicii de retur. Te rugăm să încerci din nou."
          );
        } else if (
          message
        ) {
        ...
        {localLoading ? (
          ...
          <>
            ...
            >
              ...
              <section
                className={
                  styles.vendorGateSection
                }
              >
                <h4
                  className={
                    styles.vendorGateSectionTitle
                  }
                >
                  Date pentru
                  retururi
