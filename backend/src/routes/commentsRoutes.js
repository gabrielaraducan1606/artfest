...
function sanitizeText(s, max = 1000) {
  // scoate HTML, normalizează white-space și taie lungimea
...
/** Construiește arborescență simplă (1 nivel de reply) */
function buildTree(items) {
  ...
  // ordonează fiecare nivel în ordine desc, ca feed-ul
...
/* ----------------- Create ----------------- */
/** POST /api/comments  body: { productId, text, parentId? } */
router.post("/comments", authRequired, async (req, res) => {
  const { productId, text, parentId } = req.body || {};
  const clean = sanitizeText(text);
  if (!productId || !clean) return res.status(400).json({ error: "invalid_input" });

  // produs + vendor (pentru a bloca comentariul la propriul produs)
  ...
  const meVendor = await prisma.vendor.findUnique({ where: { userId: req.user.sub }, select: { id: true } });
  ...
  if (countTotal > 50) return res.status(429).json({ error: "rate_limited" });

  // cap per produs (anti-flood pe același produs)
  const countPerProduct = await prisma.comment.count({
    where: { userId: req.user.sub, productId, createdAt: { gte: since24h } },
  });
  if (countPerProduct > 10) return res.status(429).json({ error: "rate_limited_product" });

  // anti link-spam scurt
  if (/https?:\/\//i.test(clean) && clean.length < 60) {
    return res.status(400).json({ error: "suspicious_content" });
  }

  // anti-duplicate în 2 minute pe același produs
  const since2m = nowMinus(2 * 60 * 1000);
  const duplicate = await prisma.comment.findFirst({
    where: { userId: req.user.sub, productId, text: clean, createdAt: { gte: since2m } },
    select: { id: true },
  });
  if (duplicate) return res.status(409).json({ error: "duplicate" });

  // dacă parentId este dat, validează că aparține aceluiași produs
  if (parentId) {
    const parent = await prisma.comment.findUnique({ where: { id: parentId }, select: { productId: true } });
  ...
  const saved = await prisma.comment.create({
    data: { productId, userId: req.user.sub, text: clean, parentId: parentId || null },
  });
 ...
 *  - skip/take aplicate pe comentariile rădăcină (parentId=null)
 *  - replies incluse pentru fiecare părinte
 *  - mode=flat returnează lista ordonată desc (compat vechi)
 */
router.get("/public/product/:id/comments", async (req, res) => {
  ...
  // ia toți părinții din fereastra cerută
  ...
  // ia toate replies pentru părinții de mai sus (1 nivel)
...
/* ----------------- Edit (grace 15 min) ----------------- */
/** PATCH /api/comments/:id  body: { text } — autorul în max 15 minute sau ADMIN */
router.patch("/comments/:id", authRequired, async (req, res) => {
  ...
  const me = await prisma.user.findUnique({ where: { id: req.user.sub }, select: { role: true, id: true } });
  const isOwner = comment.userId === me.id;
  const within15 = Date.now() - new Date(comment.createdAt).getTime() < 15 * 60 * 1000;
...
/* ----------------- Delete ----------------- */
/** DELETE /api/comments/:id  (owner în 24h sau ADMIN) */
router.delete("/comments/:id", authRequired, async (req, res) => {
  ...
  const me = await prisma.user.findUnique({ where: { id: req.user.sub }, select: { role: true, id: true } });
  const isOwner = comment.userId === me.id;
  const within24h = Date.now() - new Date(comment.createdAt).getTime() < 24 * 3600 * 1000;
