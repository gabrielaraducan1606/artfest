...
router.post("/me/onboarding/reset", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const vendor = await prisma.vendor.findUnique({ where: { userId: req.user.sub } });
...
router.delete("/me", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const vendor = await prisma.vendor.findUnique({ where: { userId: req.user.sub } });
  if (!vendor) return error(res, "vendor_profile_missing", 404);
 ...
 * Permite USER -> creează Vendor + promovează rolul în aceeași tranzacție.
 */
router.post("/me/services", authRequired, async (req, res) => {
  try {
    if (!req.user?.sub) {
      console.error("[/me/services] MISSING req.user. Headers:", {
    ...
    console.log("[/me/services] userId:", userId, "body:", req.body);

    // 1) Creează Vendor + promovează rolul dacă e cazul
    let meVendor = await prisma.vendor.findUnique({ where: { userId } });
    if (!meVendor) {
      meVendor = await prisma.$transaction(async (tx) => {
      ...
      console.log("[/me/services] created vendor:", meVendor.id);
    } else {
      const u = await prisma.user.findUnique({ where: { id: userId }, select: { role: true } });
      if (u && u.role !== "VENDOR") {
        await prisma.user.update({ where: { id: userId }, data: { role: "VENDOR" } });
        console.log("[/me/services] role promoted to VENDOR");
      }
    }

    // 2) Determină tipurile cerute
    ...
    console.log("[/me/services] resolved types:", types.map(t => t.code));

    if (!types.length) {
      return res.status(404).json({ error: "service_types_not_found" });
    }

    // 3) Creează/upsertează drafts
    const items = [];
  ...
  where: { vendor_type_unique: { vendorId: meVendor.id, typeId: t.id } },
   ...
   create: {
     ...
     status: "DRAFT",
     isActive: false,
     // câmpurile din schema sunt compatibile cu valorile
     // title/description/basePriceCents sunt opționale (String?/Int?), deci pot lipsi sau fi null
     coverageAreas: [],   // String[] (fără default) — trebuie setat la create (corect)
     mediaUrls: [],       // String[] (fără default) — trebuie setat la create (corect)
   },
   include: { type: true, profile: true },
    ...
    // trimitem detalii utile în răspuns ca să vezi imediat cauza
...
router.patch("/me/services/:id", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const { id } = req.params;
  const { title, description, basePriceCents, currency, city, coverageAreas, mediaUrls, attributes } = req.body || {};
...
router.put("/vendor-services/:id/profile", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  try {
    const { id } = req.params;
    const meVendor = await prisma.vendor.findUnique({ where: { userId: req.user.sub } });
    if (!meVendor) return error(res, "vendor_profile_missing", 404);
...
router.post("/me/services/:id/activate", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const { id } = req.params;
  ...
  if (Array.isArray(svc.type?.fields)) fields = svc.type.fields;
    ...
    try {
      fields = JSON.parse(svc.type.fields) || [];
    } catch {
      fields = [];
    }
  }
...
router.delete("/vendor-services/:id/profile", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const { id } = req.params;
...
router.post("/me/services/:id/deactivate", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const { id } = req.params;
...
router.delete("/me/services/:id", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const { id } = req.params;
...
router.get("/me/stats", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
 ...
 */
router.post("/store/:slug/products", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  ...
  if (!profile || profile.service.vendorId !== meVendor.id || profile.service.type?.code !== "products") {
    return error(res, "service_not_found_or_forbidden", 404);
  }

  const imgs = Array.isArray(images) ? images.filter((u) => typeof u === "string" && u.trim()) : [];
 ...
 */
router.put("/products/:id", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const { id } = req.params;
...
router.delete("/products/:id", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const { id } = req.params;
...
router.get("/products/:id", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const { id } = req.params;
  ...
  if (!p || p.service.vendorId !== meVendor.id || p.service.type?.code !== "products") {
    return res.status(404).json({ error: "product_not_found_or_forbidden" });
  }
...
router.get("/me", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
  const v = await prisma.vendor.findUnique({
    where: { userId: req.user.sub },
    select: {
...
router.patch("/me", authRequired, requireRole("VENDOR", "ADMIN"), async (req, res) => {
