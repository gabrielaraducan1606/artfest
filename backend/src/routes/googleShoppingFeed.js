  ...
  try {
    ...
    const items = products
      .map((p) => {
        ...
        const price = `${(p.priceCents / 100).toFixed(2)} ${p.currency || "RON"}`;
      ...
      <g:brand>${escapeXml(p.service?.vendor?.displayName || "Artfest")}</g:brand>
      <g:product_type>${escapeXml(p.category || "handmade")}</g:product_type>
...
<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">
  <channel>
    ...
    <link>${BASE_URL}</link>
    <description>Produse handmade și personalizate create de artizani români.</description>
${items}
  </channel>
</rss>`;
