...
router.get("/series", async (req, res) => {
  try {
    ...
    console.error("GET /visitors/series error", e);
    return res.status(500).json({ error: "internal_error" });
  }
});
...
router.get("/kpi", async (req, res) => {
  try {
    ...
    console.error("GET /visitors/kpi error", e);
    return res.status(500).json({ error: "internal_error" });
  }
});
...
router.get("/top-pages", async (req, res) => {
  try {
    ...
    const rows = await prisma.event.groupBy({
      ...
      // ✅ cel mai compatibil: numărăm pageUrl
      _count: { pageUrl: true },
      // ✅ sortăm după count(pageUrl)
    ...
    const items = rows.map((r) => {
      ...
      return {
        url,
        title,
        views: r._count.pageUrl || 0, // ✅ aici e count-ul
        avgTime: 0,
      };
    });
    ...
    console.error("GET /visitors/top-pages error", e);
    return res.status(500).json({ error: "internal_error" });
  }
});
...
router.get("/referrers", async (req, res) => {
  try {
    ...
    console.error("GET /visitors/referrers error", e);
    return res.status(500).json({ error: "internal_error" });
  }
});
...
router.get("/searches", async (req, res) => {
  try {
    ...
    console.error("GET /visitors/searches error", e);
    return res.status(500).json({ error: "internal_error" });
  }
});
...
router.get("/cta-performance", async (req, res) => {
  try {
    ...
    const convMap = new Map(
      conv.map((r) => [r.ctaLabel || "", r._count._all])
    );
    ...
    console.error("GET /visitors/cta-performance error", e);
    return res.status(500).json({ error: "internal_error" });
  }
});
...
router.get("/realtime", async (req, res) => {
  try {
    ...
    const active = new Set(rows.map((r) => r.sessionId).filter(Boolean)).size;
    return res.json({ active });
  } catch (e) {
    console.error("GET /visitors/realtime error", e);
    return res.status(500).json({ error: "internal_error" });
  }
});
...
router.get("/meta", async (req, res) => {
  try {
    // ✅ FIX: nu req.user.vendorId, ci req.vendorId
    const vendorId = req.vendorId;
    ...
    return res.json({ createdAt: vendor?.createdAt || null });
  } catch (e) {
    console.error("GET /visitors/meta error", e);
    return res.status(500).json({ error: "internal_error" });
  }
});
